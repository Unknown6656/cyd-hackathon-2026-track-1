...
- Your harness already corroborated the gemma sup-1 value (`rt-sup-1-*` sessions) — cited in `submit/02` and `report.md` §4.

## Your open lines (from `unknown6656/FINDINGS.md` "Attack surface still open")

1. sup-3/4/5 correction/disambiguation batteries on **gemma** (only Mistral fully swept) — your `attack.py` framing batteries port directly; add the three patterns that actually worked (see below) if not present.
2. Encoded tool-store payloads (base64 "legacy note") and split vendor-spec + bulletin in one session — target Mistral.
3. `model_name` edge cases are now closed (400 clean, see `probes/d_api.md`) — no need to revisit.
